...
export function stepFromStepNode(stepNode, context) {
  const { Step } = elements,
        ...
        procedureReference = procedureReferenceFromStepNode(stepNode, context),
  ...
  const step = new Step(context, string, node, breakPoint, reference, statement, procedureReference, schemaAssertion, signatureAssertion);
...
export function schemaFromSchemaNode(schemaNode, context) {
  const { Schema } = elements,
        proof = proofFromSchemaNode(schemaNode, context),
        label = labelFromSchemaNode(schemaNode, context),
        deduction = deductionFromSchemaNode(schemaNode, context),
        suppositions = suppositionsFromSchemaNode(schemaNode, context),
        constraints = constraintsFromSchemaNode(schemaNode, context),
        schemaString = schemaStringFromLabelSuppositionsAndDeduction(label, suppositions, deduction),
        node = schemaNode, ///
        string = schemaString, ///
        breakPoint = null,
...
export function hypothesisFromHypothesisNode(hypotheseNode, context) {
  const { Hypothesis } = elements,
        ...
        procedureReference = procedureReferenceFromHypothesisNode(hypotheseNode, context);

  context = null;

  const hypohtesis = new Hypothesis(context, string, node, breakPoint, statement, procedureReference);

  return hypohtesis;
}

export function constraintFromConstraintNode(constraintNode, context) {
...
export function procedureReferenceFromProcedureReferenceNode(procedureReferenceNode, context) {
  const { ProcedureReference } = elements,
        ...
        procedureReferenceString = procedureReferenceStringFromNameAndParameters(name, parameters),
        node = procedureReferenceNode, ///
        string = procedureReferenceString, ///
        breakPoint = null;

  context = null;

  const procedureReference = new ProcedureReference(context, string, node, breakPoint, name, parameters);
...
export function proofFromSchemaNode(schemaNode, context) {
  const proofNode = schemaNode.getProofNode(),
        proof = proofFromProofNode(proofNode, context);
...
export function referenceFromPremiseNode(premiseNode, context) {
  const procedureReference = null;
...
export function constraintsFromSchemaNode(schemaNode, context) {
  const constraints = [];
...
export function suppositionsFromSchemaNode(schemaNode, context) {
  const suppositionNodes = schemaNode.getSuppositionNodes(),
        suppositions = suppositionsFromSuppositionNodes(suppositionNodes, context);
...
export function referenceFromSuppositionNode(suppositionNode, context) {
  let referencel = null;
...
export function procedureReferenceFromStepNode(stepNode, context) {
  const procedureReference = null;
...
export function statementFromImplicitAssumptionNode(implicitAssumptionNode, context) {
  const statesmentNode = implicitAssumptionNode.getStatementNode(),
        statement = statementFromStatementNode(statesmentNode, context);
...
export function procedureReferenceFromHypothesisNode(hypothesisNode, context) {
  let procedureReference = null;
...
export function parametersFromProcedureReferenceNode(procedureReferenceNode, context) {
...
export function procedureReferenceFromSuppositionNode(suppositionNode, context) {
  let procedureReference = null;
